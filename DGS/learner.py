...
def run(rank, workers, model, save_path, train_data, test_data):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_data):
            # 在指定iterations减少学习率缩放因子
            if (global_clock + 1) in [0,1]:
                ratio = ratio * 0.1
                print(ratio)
            ...
            for idx, param in enumerate(model.parameters()):
                ...
                param.data = recv
            threshold_recv = torch.tensor([0.0])
            dist.scatter(tensor=threshold_recv, src=0, group=group)
            threshold = threshold_recv.data.item()
