...
# 模型与数据集
parser.add_argument('--data-dir', type=str, default='/home/centos/dataset')
...
def run(rank, model, train_pics, train_bsz):
    ...
    g_list = [[torch.zeros_like(param.data) for param in model.parameters()] for _ in range(len(workers)+1)]
    ...
    for epoch in range(args.epochs):
        for batch_idx, (_, _) in enumerate(tmp):
            ...
            sparsification_ratio_list = [torch.tensor([0.0]) for _ in range(len(workers) + 1)]
            dist.gather(tensor=torch.tensor([0.0]), gather_list=sparsification_ratio_list, group=group)
            sparsification_ratio += sum(sparsification_ratio_list).item()/len(workers)
            ...
            for layer_idx, param in enumerate(model.parameters()):
                ...
                g_layer_temp = torch.zeros_like(param.data)
                for worker_idx, w_g in enumerate(g_list):
                    w_g[layer_idx] += gather_list[worker_idx]
                    g_layer_temp += w_g[layer_idx]

                g_avg = g_layer_temp  / len(workers)
                param.data -= g_avg
                g_quare_sum += torch.sum(g_avg * g_avg)

                scatter_list = [param.data for _ in range(len(workers) + 1)]
                dist.scatter(tensor=g_avg, scatter_list=scatter_list, group=group)
                param_idx += 1

            g_quare_sum = torch.sqrt(g_quare_sum)/len(workers)
            g_quare_sum_list = [torch.tensor([0.0])+g_quare_sum for _ in range(len(workers) + 1)]
            dist.scatter(tensor=torch.tensor([0.0]), scatter_list=g_quare_sum_list, group=group)
            global_clock += 1
